...
class CarrierConnectRequestsModel extends BaseModel{
	...
	public function format($row = false){
		...
		if($row){
			...
			$row->didit_verified = false;

			if($row->didit_status == 'Approved'){

				$row->didit_verified = true;
			}

			$row->bank_verified = false;

			if($row->stripe_verified_at != null){

				$row->bank_verified = true;
			}

			$row->doc_url = $this->file_url('sample-doc.pdf', 'uploads/doc/');
			...
			unset($row->didit_session_id);
			unset($row->didit_response);
			unset($row->didit_reponse_at);
		}
	...
	public function connect_stripe_express_account($request, $user){

		$row_id = $request->post('row_id');

        $stripe_model = new StripeModel;

		$_connect_request = $this->fetch_row_by_id($row_id);

        if($_connect_request){

			$load_carrier = $this->load_carrier($_connect_request->receiver_id);

			if($load_carrier['status']){

				$carrier = $load_carrier['row'];

				$response = $stripe_model->createExpressAccount($carrier);
				...
				if($response['status'] == true){
					...
					Update carrier data
					*/
					$this->carriers_model->set_post_data('stripe_express_account', $account_id);
					$this->carriers_model->post_update($carrier->row_id);
					...
					*/
					...
					$_response = $stripe_model->createOnboardingLink($account_id, $row_id);
					...
					return ['status' => false, 'error' => "There was an error while setting up stripe express account."];
				}
			}else{

				return ['status' => false, 'error' => "There was an error fetching carrier details."];
			}
		}else{

			return ['status' => false, 'error' => "Connect request not found."];
		}
	}

	public function verify_stripe_account($request, $user){
        ...
        if($_connect_request){
			...
			$stripe_express_account = $_connect_request->stripe_express_account;

			if($stripe_express_account){

				$response = $stripe_model->expressAccountConnectStatus($stripe_express_account);

				if($response['status'] == true){

					/*
					Update request connect
					*/
					$this->set_post_data('stripe_verified_at', date('Y-m-d H:i:s'));
					$this->post_update($row_id);
					...
					return ['status' => true, 'message' => $response['message'], 'connect_request' => $_request];
				}

				return ['status' => false, 'message' => $response['message']];
			}else{

				return ['status' => false, 'message' => 'Stripe account not connected.'];
			}
		}
	...
	public function carrier(){
     	return $this->belongsTo(Carrier::class, 'receiver_id', 'row_id');
    	}

	public function broker_carrier_connect_list($request, $user){
		//dd( $user);
        $query = self::with(['carrier'])->where('sender_id', $user['row_id'])->orderBy('id', 'desc');
        //$query = $query->get();
        return $query;
	}

	public function exportconnect_listCsv($request, $user){
		$fileName = 'connect_list_' . now()->format('Ymd_His') . '.csv';
		...
		$connect_list = self::with('carrier')->where('sender_id', $user['row_id'])->orderByDesc('id')->get();
		...
		foreach ($connect_list as $list) {
			fputcsv($file, [
				$list->carrier?->dba_name ?? '',
				$list->carrier?->legal_name ?? '',
				$list->carrier?->dot_number ?? '',
				$list->carrier?->MC ?? '',
				$list->didit_status ?? '',
			]);
		}
		...
		return asset('storage/exports/' . $fileName);
		// return [
		// 'status' => true,
		// 'file_name' => $fileName,
	...
	public function exportConnect_listPdf($request, $user){
		$connect_list = self::with('carrier')->where('sender_id', $user['row_id'])->orderByDesc('id')->get();

		$pdf = Pdf::loadView('pdf.broker-carrier-connect-list', [
			'connect_list' => $connect_list
		]);

		$fileName = 'connect_list_' . now()->format('Ymd_His') . '.pdf';
